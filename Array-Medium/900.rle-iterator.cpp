...
public:
    ...
    int next(int n) {
        ...
        for(int i=n;i>0;){//i--的操作就不需要了
            //一个一个删会超过时间限制
            while (head<sequence.size()&&sequence[head]==0)
            {
                head=head+2;
            }
            if(head>=sequence.size()){//注意这里有等号，否则会超出数组范围
                return -1;
            }else{
                if(i<=sequence[head]){
                    //当前元素够减的

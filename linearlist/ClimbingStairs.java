...
public class ClimbingStairs {
	...
	public static int climbStairsDinamicPro(int n) {
		/*
		 * 初始递归问题再于一个问题，如爬3层楼梯会重复计算很多次，没有充分利用每次计算出的中间问题，造成了浪费 动态规划
	...
	public static int climbStairsEhancedVersion(int n) {
		...
		if (n == 1 || n == 0) {
			...
			int mid = n / 2;
			return (n % 2 == 0)
					? climbStairsEhancedVersion(mid) * climbStairsEhancedVersion(mid)
							+ climbStairsEhancedVersion(mid - 1) * climbStairsEhancedVersion(mid - 1)
					: climbStairsEhancedVersion(mid + 1) * climbStairsEhancedVersion(mid)
							+ climbStairsEhancedVersion(mid) * climbStairsEhancedVersion(mid - 1);
		}
